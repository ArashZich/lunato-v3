    ...
    try:
        ...
        if not os.path.exists(cascade_path):
            ...
            raise FileNotFoundError(
                f"فایل مدل تشخیص چهره یافت نشد: {cascade_path}")
...
def load_landmark_detector():
    ...
    try:
        ...
        try:
            import dlib
            _landmark_detector = dlib.shape_predictor(
                "data/shape_predictor_68_face_landmarks.dat")
            logger.info("مدل تشخیص نقاط کلیدی dlib با موفقیت بارگیری شد")
        except (ImportError, RuntimeError):
            logger.warning(
                "امکان استفاده از مدل dlib وجود ندارد. استفاده از روش جایگزین...")
...
# app/core/face_detection.py را با این کد بهبود یافته جایگزین کنید (بخش detect_face)

def detect_face(image: np.ndarray) -> Dict[str, Any]:
    """
    تشخیص چهره در تصویر با استفاده از روش‌های متعدد.
    ...
    """
    try:
        # 1. ابتدا از Mediapipe استفاده می‌کنیم (دقیق‌تر اما کندتر)
        try:
            import mediapipe as mp
            ...
            with mp_face_detection.FaceDetection(
                model_selection=1,  # مدل با دقت بیشتر
                min_detection_confidence=0.5
            ) as face_detection:
                results = face_detection.process(rgb_image)

                if results.detections and len(results.detections) > 0:
                    # از اولین چهره تشخیص داده شده استفاده می‌کنیم
                    detection = results.detections[0]

                    # تبدیل مختصات نسبی به مختصات تصویر
        ...
        try:
            # مسیرهای مدل DNN
            model_path = "data/face_detection_models/opencv_face_detector_uint8.pb"
            config_path = "data/face_detection_models/opencv_face_detector.pbtxt"
            ...
            if os.path.exists(model_path) and os.path.exists(config_path):
                ...
                # یافتن چهره با بیشترین اطمینان
                for i in range(detections.shape[2]):
                    confidence = detections[0, 0, i, 2]

                    if confidence > 0.7:  # حد آستانه بالاتر برای دقت بیشتر
                ...
                if max_confidence_idx >= 0:
                    # استخراج مختصات چهره
                    box = detections[0, 0, max_confidence_idx, 3:7] * np.array(
        ...
        # تشخیص چهره با پارامترهای بهبود یافته
            ...
            with mp_face_mesh.FaceMesh(
                ...
                if results.multi_face_landmarks:
                    ...
                    selected_landmarks = np.array([
                        ...
                        [(landmarks[70].x * face_width + x1),
                         (landmarks[70].y * face_height + y1)],   # پیشانی چپ
                        [(landmarks[8].x * face_width + x1),
                         (landmarks[8].y * face_height + y1)],     # وسط پیشانی
                        [(landmarks[300].x * face_width + x1),
                         (landmarks[300].y * face_height + y1)],  # پیشانی راست

                        # فک - استفاده از نقاط دقیق‌تر برای تشخیص عرض فک
                        [(landmarks[172].x * face_width + x1),
                         (landmarks[172].y * face_height + y1)],   # فک چپ
                        [(landmarks[152].x * face_width + x1),
                         (landmarks[152].y * face_height + y1)],   # چانه
                        [(landmarks[397].x * face_width + x1),
                         (landmarks[397].y * face_height + y1)],   # فک راست

                        # گونه‌ها - استفاده از نقاط استخوان گونه
                        [(landmarks[234].x * face_width + x1),
                         (landmarks[234].y * face_height + y1)],   # گونه چپ
                        [(landmarks[1].x * face_width + x1),
                         # بینی (وسط)
                         (landmarks[1].y * face_height + y1)],
                        [(landmarks[454].x * face_width + x1),
                         (landmarks[454].y * face_height + y1)]    # گونه راست
                    ])

                    logger.info(
                        f"تشخیص {len(selected_landmarks)} نقطه کلیدی با استفاده از mediapipe")
...
def _generate_dynamic_landmarks(image: np.ndarray, face_coordinates: Dict[str, int]) -> np.ndarray:
    ...
    logger.info(
        f"نقاط کلیدی پویا: نسبت پیشانی به گونه: {forehead_to_cheekbone:.2f}")
...
def get_face_image(image: np.ndarray) -> Tuple[bool, Dict[str, Any], Optional[np.ndarray]]:
    ...
    try:
        ...
        if width < 100 or height < 100:
            logger.warning(
                f"تصویر با ابعاد {width}x{height} کیفیت مناسبی برای تشخیص چهره ندارد")
            return False, {
                "success": False,
        ...
        if not detection_result.get("success", False):
            logger.warning(
                f"تشخیص چهره ناموفق بود: {detection_result.get('message', 'دلیل نامشخص')}")
        ...
        if aspect_ratio < 0.5 or aspect_ratio > 2.0:
            logger.warning(
                f"نسبت ابعاد چهره ({aspect_ratio:.2f}) خارج از محدوده منطقی است")
            return False, {
                "success": False,
        ...
        logger.info(
            f"چهره با موفقیت تشخیص داده شد. ابعاد تصویر برش خورده: {x2-x1}x{y2-y1}")
