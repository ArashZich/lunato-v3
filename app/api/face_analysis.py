...
async def analyze_face(
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=400, detail="فایل تصویر نامعتبر است")
        ...
        # پردازش همزمان
            ...
            if image is None:
                raise HTTPException(
                    status_code=400, detail="خطا در خواندن فایل تصویر")
            ...
            response = FaceAnalysisResponse(
                ...
                client_info=client_info,
                recommended_frame_types=analysis_result.get(
            ...
            if include_frames:
                ...
                if frames_result.get("success", False):
                    await save_recommendation(
                        user_id=user_id,
                        face_shape=face_shape,
                        recommended_frame_types=frames_result.get(
                            "recommended_frame_types", []),
                        recommended_frames=frames_result.get(
                            "recommended_frames", []),
                        client_info=client_info_dict,
                ...
                response.recommended_frames = frames_result.get(
                    "recommended_frames", [])
        ...
        raise HTTPException(
            status_code=500, detail=f"خطا در پردازش درخواست: {str(e)}")
...
async def get_analysis_status(task_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"خطا در بررسی وضعیت پردازش: {str(e)}")


@router.get("/face-shapes/{face_shape}/frames", response_model=FaceAnalysisResponse)
async def get_frames_for_face_shape(
    # استفاده از Enum
    face_shape: FaceShapeEnum = Path(..., description="شکل چهره"),
    min_price: Optional[float] = Query(None, description="حداقل قیمت (تومان)"),
    max_price: Optional[float] = Query(
        None, description="حداکثر قیمت (تومان)"),
    limit: int = Query(
        15, description="حداکثر تعداد فریم‌های پیشنهادی"),  # تغییر به 15
    request: Request = None
):
    """
    ...
    try:
        ...
        if result.get("success", False):
            await save_recommendation(
                user_id=user_id,
                face_shape=face_shape,
                recommended_frame_types=result.get(
                    "recommended_frame_types", []),
                recommended_frames=result.get("recommended_frames", []),
                client_info=client_info_dict
            )
