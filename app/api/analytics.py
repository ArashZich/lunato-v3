...
async def get_analytics_summary_api(
    period: Optional[str] = Query(
        "all", description="دوره زمانی (today, week, month, all)")
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"خطا در دریافت اطلاعات تحلیلی: {str(e)}")
...
async def get_detailed_analytics_api(
    period: Optional[str] = Query(
        "all", description="دوره زمانی (today, week, month, all)"),
    skip: int = Query(
        0, description="تعداد رکوردهای نادیده گرفته شده (برای صفحه‌بندی)"),
...
async def get_time_based_analytics_api(
    group_by: str = Query(
        "day", description="گروه‌بندی بر اساس زمان (hour, day, week, month)"),
    period: Optional[str] = Query(
        "month", description="دوره زمانی (today, week, month, all)"),
    face_shape: Optional[str] = Query(
        None, description="فیلتر بر اساس شکل چهره")
):
    """
...
async def get_frame_popularity_api(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"خطا در دریافت محبوب‌ترین فریم‌ها: {str(e)}")
...
async def get_conversion_stats_api(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"خطا در دریافت آمار تبدیل: {str(e)}")
